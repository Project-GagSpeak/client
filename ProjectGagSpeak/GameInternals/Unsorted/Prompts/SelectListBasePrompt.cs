...
public abstract class SetupSelectListPrompt : BasePrompt
{
    ...
    {
        var nodes = MainConfigService.GetAllNodes().OfType<TextEntryNode>();
            ...
            if (node.TargetRestricted)
            {
                ...
                if (!string.IsNullOrEmpty(node.TargetNodeLabel))
                {
                    if (EntryMatchesTargetName(node, nodeLabel))
                    {
                        ...
                        MainConfigService.LastSelectedListNode = node;
                        return index;
                    }
                }
                // Fallback to using LastSeenNodeName if TargetNodeLabel is not provided
                else if (!string.IsNullOrEmpty(MainConfigService.LastSeenNodeName))
                {
                    if (string.Equals(node.TargetNodeName, MainConfigService.LastSeenNodeName, StringComparison.OrdinalIgnoreCase))
                    {
