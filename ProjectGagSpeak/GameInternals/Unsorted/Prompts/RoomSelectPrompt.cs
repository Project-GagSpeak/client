...
public class RoomSelectPrompt : BasePrompt
{
    ...
    internal RoomSelectPrompt(ILogger<RoomSelectPrompt> logger, MainConfigService config, 
    ...
    private unsafe void SetEntry(AddonEvent type, AddonArgs args)
    {
        try
        {
            ...
            MainConfigService.LastSeenNodeLabel = AddonBaseRoom.ToText(addon, 8);
        }
        catch { }
    }
    ...
    protected async void AddonSetup(AddonEvent eventType, AddonArgs addonInfo)
    {
        ...
        var nodes = MainConfigService.GetAllNodes().OfType<ChambersTextNode>();
        foreach (var node in nodes)
        {
            ...
            if (node.TargetRestricted && !MainConfigService.LastSeenNodeName.Contains(node.TargetNodeName))
                continue;
