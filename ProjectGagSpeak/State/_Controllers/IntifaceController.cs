...
public class IntifaceController : DisposableMediatorSubscriberBase
{
    ...
    private void OnDeviceRemoved(ButtplugClientDevice removedDevice)
    {
        try
        {
            ...
            if (IdxToRemove > -1)
            {
                ...
                Logger.LogInformation($"Device " + _devices[IdxToRemove] + " removed from device list.", LoggerType.Toys);
                // create shallow copy
        ...
        {
            Logger.LogError($"Error removing device from device list. " + ex.Message, LoggerType.Toys);
        }
    }
    ...
    {
        Logger.LogInformation("Intiface Central Disconnected", LoggerType.Toys);
        HandleDisconnect();
    }
    ...
    public async void ConnectToIntifaceAsync()
    {
        try
        {
            ...
            {
                Logger.LogInformation("Already connected to Intiface Central", LoggerType.Toys);
                return;
            }
            ...
            Logger.LogDebug("Attempting connection to Intiface Central", LoggerType.Toys);
    ...
    public async void DisconnectFromIntifaceAsync()
    {
        try
        {
            ...
            if (ButtPlugClient.Connected)
            {
                ...
                if (!ButtPlugClient.Connected)
                {
                    Logger.LogInformation("Disconnected from Intiface Central", LoggerType.Toys);
                    ScanningForDevices = false;
    ...
    {
        Logger.LogDebug("Client was properly disconnected from Intiface Central. Disconnecting Device Handler.", LoggerType.Toys);
        try
        {
            _devices.Clear();
        }
    ...
    private async Task BatteryHealthCheck(CancellationToken ct)
    {
        while (!ct.IsCancellationRequested && ConnectedToIntiface)
        {
            ...
            Logger.LogTrace("Scheduled Battery Check on connected devices", LoggerType.Toys);
    ...
    public async Task StartDeviceScanAsync()
    {
        ...
        Logger.LogDebug("Now actively scanning for new devices...", LoggerType.Toys);
        try
        {
    ...
    public async Task StopDeviceScanAsync()
    {
        ...
        Logger.LogDebug("Halting the scan for new devices to add", LoggerType.Toys);
        try
        {
    ...
    public void ExecuteVibeTrigger(SexToyAction sexToyAction)
    {
        ...
        Logger.LogInformation("Vibe Trigger Function Accessed. This would normally play a vibe by now!", LoggerType.Toys);
    }
