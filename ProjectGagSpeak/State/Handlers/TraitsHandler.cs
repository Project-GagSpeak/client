...
public class TraitsHandler
{
    ...
    public Task UpdateTraitCache()
    {
        ...
        if (_cache.UpdateFinalCache())
            _logger.LogDebug("Final Traits updated.", LogFilter.VisualCache);
        else
            _logger.LogTrace("No change in Final Traits.", LogFilter.VisualCache);
        _controller.UpdateHardcoreStatus();
        _mediator.Publish(new HcStateCacheChanged());
        _logger.LogDebug("Finished Updating Traits Caches.", LogFilter.VisualCache);
