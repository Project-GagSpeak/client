...
public sealed partial class GlamourHandler
{
    ...
    private GlamourActorCache                  _latestUnboundState = GlamourActorCache.Empty;
    ...
    #region Semiphore Appliers
    public async Task ApplySemaphore(CacheType taskFlags)
    {
        if (taskFlags is CacheType.None)
            return;

        await ExecuteWithSemaphore(async () =>
        {
            var tasks = new List<Task>();
            if ((taskFlags & CacheType.Glamour) != 0)
                tasks.Add(ApplyGlamourCache());

            if ((taskFlags & CacheType.Meta) != 0)
                tasks.Add(ApplyMetaCache());

            if ((taskFlags & CacheType.Customize) != 0)
                tasks.Add(ApplyCustomizeCache());

            _logger.LogInformation($"Updating Caches via Semaphore Execution with flags: {taskFlags}");
            await Task.WhenAll(tasks);
        });
    }

    public async Task RemoveSemaphore(CacheType taskFlags, List<EquipSlot>? removedSlots = null)
    {
        if(((taskFlags & CacheType.Glamour) != 0) && removedSlots is null)
        {
        ...
        await ExecuteWithSemaphore(async () =>
        {
            var tasks = new List<Task>();
            if ((taskFlags & CacheType.Glamour) != 0)
                tasks.Add(RestoreAndReapply(removedSlots!));
