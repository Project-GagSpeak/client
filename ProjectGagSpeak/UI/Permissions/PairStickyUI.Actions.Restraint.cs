            ...
            using (var actionChild = ImRaii.Child("SetApplyChild", new Vector2(WindowMenuWidth, ImGui.GetFrameHeight()), false))
            {
                ...
                (onButtonPress) =>
                {
                    try
                    {
                        ...
                        _ = _apiHubMain.UserPushPairDataWardrobeUpdate(new(StickyPair.UserData, newWardrobe, MainHub.PlayerUserData, WardrobeUpdateType.RestraintApplied, newWardrobe.Padlock.ToPadlock()));
                ...
                (onButtonPress) =>
                {
                    try
                    {
                        ...
                        if (res.Item1)
                        {
                            ...
                            _ = _apiHubMain.UserPushPairDataWardrobeUpdate(new(StickyPair.UserData, newWardrobeData, MainHub.PlayerUserData, WardrobeUpdateType.RestraintLocked, Padlocks.None));
                ...
                if (_uiShared.IconTextButton(FontAwesomeIcon.Unlock, "Unlock", ImGui.GetContentRegionAvail().X, false, disabled))
                {
                    try
                    {
                        ...
                        if (res.Item1)
                        {
                            ...
                            newWardrobeData.Assigner = MainHub.UID;
                            _ = _apiHubMain.UserPushPairDataWardrobeUpdate(new(StickyPair.UserData, newWardrobeData, MainHub.PlayerUserData, WardrobeUpdateType.RestraintUnlocked, prevLock));
            ...
            using (var actionChild = ImRaii.Child("SetRemoveChild", new Vector2(WindowMenuWidth, ImGui.GetFrameHeightWithSpacing()), false))
            {
                ...
                if (ImGui.Button("Remove Restraint", ImGui.GetContentRegionAvail()))
                {
                    try
                    {
                        ...
                        _ = _apiHubMain.UserPushPairDataWardrobeUpdate(new(StickyPair.UserData, newWardrobeData, MainHub.PlayerUserData, WardrobeUpdateType.RestraintDisabled, Padlocks.None));
