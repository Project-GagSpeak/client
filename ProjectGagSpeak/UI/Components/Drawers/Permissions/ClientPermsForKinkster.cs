using CkCommons.Gui;using CkCommons.Raii;using Dalamud.Interface.Utility.Raii;using GagSpeak.Kinksters;using GagSpeak.PlayerClient;using GagSpeak.Services;using GagSpeak.Utils;using GagSpeak.WebAPI;using GagspeakAPI.Data.Permissions;using GagspeakAPI.Extensions;using ImGuiNET;using OtterGui;using OtterGui.Text;using System.Collections.Immutable;namespace GagSpeak.Gui.Components;
        ...
        if (button)
        {
            ...
            UiService.SetUITask(res.type switch
            {
                PermissionType.PairAccess
                    => _hub.UserChangeOwnPairPermAccess(new(MainHub.PlayerUserData, new KeyValuePair<string, object>(res.name, newState), UpdateDir.Own, MainHub.PlayerUserData)),
                PermissionType.PairPerm
                    => _hub.UserChangeOwnPairPerm(new(MainHub.PlayerUserData, new KeyValuePair<string, object>(res.name, newState), UpdateDir.Own, MainHub.PlayerUserData)),
                _ when OwnGlobals.Perms != null
                    => _hub.UserChangeOwnGlobalPerm(new(MainHub.PlayerUserData, new KeyValuePair<string, object>(res.name, newState), UpdateDir.Own, MainHub.PlayerUserData)),
    ...
    private void DrawHcPermRow(Kinkster kinkster, string dispName, float width, SPPID perm, string current, bool canUse)
    {
        ...
        using (ImRaii.Disabled(kinkster.OwnPerms.InHardcore))
        {
            ...
            if (button)
            {
                ...
                //    _ when OwnGlobals.Perms is { } perms => PermissionHelper.ChangeOwnGlobal(_hub, perms, res.name, !canUse),
                //    _ => Task.CompletedTask
                //});
            }
        }
    ...
    private readonly ImmutableDictionary<SPPID, PermDataClient> ClientPermData = ImmutableDictionary<SPPID, PermDataClient>.Empty
        ...
        .Add(SPPID.GaggedNameplate,       new PermDataClient(FAI.IdCard,                FAI.Ban,           "enabled",       "disabled",        "Gag Nameplates",         "to enable gagged nameplates.",     "from enabling gagged nameplates."))
        ...
        .Add(SPPID.ApplyLayers,           new PermDataClient(FAI.LayerGroup,            FAI.Ban,           "allowed",       "restricted",      "Applying Layers",          "to apply layers",                  "from applying layers"))
        .Add(SPPID.ApplyLayersWhileLocked,new PermDataClient(FAI.LayerGroup,            FAI.Ban,           "allowed",       "restricted",      "Applying Layers While Locked", "to apply layers while locked",   "from applying layers while locked"))
        ...
        .Add(SPPID.RemoveLayers,          new PermDataClient(FAI.Eraser,                FAI.Ban,           "allowed",       "restricted",      "Removing Layers",          "to remove layers",                 "from removing layers"))
        .Add(SPPID.RemoveLayersWhileLocked,new PermDataClient(FAI.Eraser,                FAI.Ban,           "allowed",       "restricted",      "Removing Layers While Locked", "to remove layers while locked", "from removing layers while locked"))
        ...
        .Add(SPPID.HypnoticEffect,        new PermDataClient(FAI.CameraRotate,          FAI.Ban,           "allowed",       "restricted",      "Hypnotic Effect",          "to send hypnotic effects",         "from sending hypnotic effects"))
