        ...
        using (var group = ImRaii.Group())
        {
            ...
            if (_uiShared.IconTextButton(FontAwesomeIcon.Sync, "Refresh", null, false, DateTime.UtcNow - LastRefresh < TimeSpan.FromSeconds(15) || !UniqueShockCollarPermsExist()))
            {
                ...
                Task.Run(async () =>
                {
                    ...
                    _ = _hub.UserPushAllUniquePerms(new(StickyPair.UserData, MainHub.PlayerUserData, OwnPerms, StickyPair.OwnPermAccess, UpdateDir.Other));
                });
            }
        }
            ...
            case PermissionType.Global:
                {
                    ...
                    _ = _hub.UserUpdateOwnGlobalPerm(new(MainHub.PlayerUserData, MainHub.PlayerUserData, new KeyValuePair<string, object>(permissionName, newValue), UpdateDir.Own));
                }
                break;
            case PermissionType.UniquePairPerm:
                {
                    _logger.LogTrace($"Updated own pair permission: {permissionName} to {newValue}", LoggerType.Permissions);
                    _ = _hub.UserUpdateOwnPairPerm(new(StickyPair.UserData, MainHub.PlayerUserData, new KeyValuePair<string, object>(permissionName, newValue), UpdateDir.Own));
                }
                break;
            // this case should technically never be called for this particular instance.
            case PermissionType.UniquePairPermEditAccess:
                {
                    ...
                    _ = _hub.UserUpdateOwnPairPermAccess(new(StickyPair.UserData, MainHub.PlayerUserData, new KeyValuePair<string, object>(permissionName, newValue), UpdateDir.Own));
                }
                break;
        }
    }
