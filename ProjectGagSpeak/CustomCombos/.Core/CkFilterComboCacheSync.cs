...
public abstract class CkFilterComboCacheSync<T> : CkFilterComboBase<T>
{
    ...
    protected override void DrawList(float width, float itemHeight)
    {
        ...
        if (NewSelection != null && Items.Count > NewSelection.Value)
            if(!ReferenceEquals(CurrentSelection, Items[NewSelection.Value]))
                ItemSelected?.Invoke(CurrentSelection, Items[NewSelection.Value]);
    }
    ...
    public virtual bool Draw(string label, string preview, string tooltip, float previewWidth, float itemHeight,
