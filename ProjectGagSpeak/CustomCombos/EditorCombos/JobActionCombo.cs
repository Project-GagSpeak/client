...
public sealed class JobActionCombo : CkFilterComboCache<ParsedActionRow>
{
    ...
    protected override bool DrawSelectable(int globalIdx, bool selected)
    {
        ...
        using (ImRaii.Group())
        {
            ...
            using (ImRaii.Group())
            {
                ImGui.Image(LociIcon.GetGameIconOrEmpty(parsedJobAction.IconID).Handle, new Vector2(size.Y));
                CkGui.TextFrameAlignedInline(parsedJobAction.Name);
                CkGui.ColorTextFrameAlignedInline($"({parsedJobAction.ActionID})", CkCol.LChildBg.Uint(), false);
            }
        }
        return ret;
    }
