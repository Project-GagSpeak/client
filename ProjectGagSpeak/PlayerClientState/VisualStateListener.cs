...
public sealed class VisualStateListener : DisposableMediatorSubscriberBase
{
    ...
    public VisualStateListener(
        ...
        AliasConfigService aliasConfig,
        IpcManager interop,
        PairManager pairs,
    ...
    public void UpdateStoredAliasName(string pairName, string listenerName)
    {
        if (!_aliasConfig.Current.AliasStorage.ContainsKey(pairName))
            _aliasConfig.Current.AliasStorage[pairName] = new AliasStorage();
        // set the name.
        _aliasConfig.Current.AliasStorage[pairName].CharacterNameWithWorld = listenerName;
        _aliasConfig.Save();

        // grab the list of names to update the chatbox listeners with.
        var newNameList = _aliasConfig.Current.AliasStorage
            .Where(x => x.Value.CharacterNameWithWorld != string.Empty)
            .Select(x => x.Value.CharacterNameWithWorld);

        Mediator.Publish(new UpdateChatListeners(newNameList));
    }
    ...
    public async Task ApplyGag(CallbackGagDataDto gagData)
    {
        ...
        if (changes.HasFlag(VisualUpdateFlags.Weapon))     await TryUpdateMetaState(MetaIndex.WeaponState, visualGagItem.WeaponState, ManagerPriority.Gags);
...
/*        foreach (var item in items)
    ...
    {
        var higherPriorityMods = new HashSet<Mod>(ManagerCacheRef.Where(entry => entry.Key > priority).SelectMany(entry => entry.Value.Mods.Keys));
        if(mods.Where(mod => higherPriorityMods.Contains(mod.ModInfo)) is { } newMods)
        {
    ...
    private void TryUpdateOrRemoveMod(IEnumerable<ModAssociation> mods)
    {
        ...
        var activeModsByInfo = ManagerCacheRef
            .SelectMany(kv => kv.Value.Mods)
            .GroupBy(g => g.Key)
            .ToDictionary(group => group.Key, group => group.Last().Value);

        foreach (var mod in mods)
        {
            if (activeModsByInfo.ContainsKey(mod.ModInfo)) // Slightly more optimized than TryGetValue due to direct access.
                _modApplier.SetOrUpdateTempMod(activeModsByInfo[mod.ModInfo]);
            else
                _modApplier.RemoveTempMod(mod);
        }
    }
