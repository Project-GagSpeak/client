...
public sealed class ClientDataListener : IDisposable
{
    ...
    public void Hypnotize(UserData enactor, HypnoticEffect effect, DateTimeOffset expireTime, string? customImage)
    {
        ...
        // Make the change
        _data.SetHardcoreStatus(enactor, HcAttribute.HypnoticEffect, newData, kinkster);
    ...
    public void ChangeAllGlobalPerms(GlobalPerms newGlobals)
    {
        ...
        HandleGlobalPermChanges(MainHub.OwnUserData, prevGlobals, ClientData.Globals);
    ...
    public void ChangeHardcoreStatus(UserData enactor, HcAttribute attribute, HardcoreStatus newData)
    {
        ...
        HandleHardcoreStatusChange(enactor, attribute, prevState, ClientData.Hardcore.IsEnabled(attribute));
    ...
    private void HandleHardcoreStatusChange(UserData enactor, HcAttribute changed, bool prevState, bool newState)
    {
        ...
        switch (changed)
        {
            ...
            case HcAttribute.Confinement:
                if (!prevState && newState) _handler.EnableConfinement(enactor, AddressBookEntry.FromHardcoreStatus(ClientData.Hardcore!));
