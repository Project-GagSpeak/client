...
public partial class AchievementManager
{
    ...
    public void InitializeAchievements()
    {
        ...
        LatestCache.SaveData.AddConditionalProgress(AchievementModuleKind.Gags, Achievements.SilentButDeadly, 10,
            () => _gags.ServerGagData is { } gagData && gagData.AnyGagActive(), (id, name) => WasCompleted(id, name).ConfigureAwait(false), "Roulettes Completed");
        ...
        LatestCache.SaveData.AddConditional(AchievementModuleKind.Gags, Achievements.SilenceOfShame, () => _gags.ServerGagData?.IsGagged() ?? false, (id, name) => WasCompleted(id, name).ConfigureAwait(false), "Kinksters", "Hushed by");

        LatestCache.SaveData.AddConditionalProgress(AchievementModuleKind.Gags, Achievements.YourFavoriteNurse, 20,
            () => _gags.ServerGagData is { } gagData && gagData.GagSlots.Any(x => x.GagItem == GagType.MedicalMask), (id, name) => WasCompleted(id, name).ConfigureAwait(false), "Patients Serviced", reqBeginAndFinish: false);

        LatestCache.SaveData.AddConditionalProgress(AchievementModuleKind.Gags, Achievements.SayMmmph, 1, () => _gags.ServerGagData?.IsGagged() ?? false, (id, name) => WasCompleted(id, name).ConfigureAwait(false), "Photos Taken");
        ...
        LatestCache.SaveData.AddConditionalThreshold(AchievementModuleKind.Wardrobe,Achievements.CrowdPleaser, 15,
            () => _restraints.AppliedRestraint is not null, (id, name) => WasCompleted(id, name).ConfigureAwait(false), "People Nearby");
        LatestCache.SaveData.AddConditionalThreshold(AchievementModuleKind.Wardrobe,Achievements.Humiliation, 5,
            () => _restraints.AppliedRestraint is not null, (id, name) => WasCompleted(id, name).ConfigureAwait(false), "GagSpeak Pairs Nearby");
        ...
        LatestCache.SaveData.AddTimeLimitedConditional(AchievementModuleKind.Wardrobe,Achievements.Bondodge,
            TimeSpan.FromSeconds(2), () => _restraints.AppliedRestraint is not null, DurationTimeUnit.Seconds, (id, name) => WasCompleted(id, name).ConfigureAwait(false));
...
/*                if ((_traits.FinalTraits & Traits.Blindfolded) != 0)
        ...
        LatestCache.SaveData.AddConditionalProgress(AchievementModuleKind.Generic, Achievements.WarriorOfLewd, 1,
            () => (_gags.ServerGagData?.IsGagged() ?? false) && _restraints.AppliedRestraint is not null, (id, name) => WasCompleted(id, name).ConfigureAwait(false), suffix: "Cutscenes Watched Bound & Gagged");

        LatestCache.SaveData.AddConditional(AchievementModuleKind.Generic, Achievements.EscapingIsNotEasy,
            () => _restraints.AppliedRestraint is not null, (id, name) => WasCompleted(id, name).ConfigureAwait(false), "Escape Attempts Made");

        LatestCache.SaveData.AddConditional(AchievementModuleKind.Generic, Achievements.ICantBelieveYouveDoneThis,
            () => _restraints.AppliedRestraint is not null, (id, name) => WasCompleted(id, name).ConfigureAwait(false), "Slaps Received");
        ...
        LatestCache.SaveData.AddConditionalProgress(AchievementModuleKind.Secrets, Achievements.KinkyTeacher, 10, () => _restraints.AppliedRestraint is not null, (id, name) => WasCompleted(id, name).ConfigureAwait(false), "Thanks Received", reqBeginAndFinish: false, isSecret: true);
        LatestCache.SaveData.AddConditionalProgress(AchievementModuleKind.Secrets, Achievements.KinkyProfessor, 50, () => _restraints.AppliedRestraint is not null, (id, name) => WasCompleted(id, name).ConfigureAwait(false), "Thanks Received", reqBeginAndFinish: false, isSecret: true);
        LatestCache.SaveData.AddConditionalProgress(AchievementModuleKind.Secrets, Achievements.KinkyMentor, 100, () => _restraints.AppliedRestraint is not null, (id, name) => WasCompleted(id, name).ConfigureAwait(false), "Thanks Received", reqBeginAndFinish: false, isSecret: true);
